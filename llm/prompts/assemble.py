"""Assemble full LLM prompts for patient and general routes."""

from llm.prompts.context import patient_context_block
from llm.prompts.schema import LLM_ANSWER_JSON_SCHEMA
...
def assemble_patient_synthesis_prompt(summary: dict, query: str) -> str:
    ...
    """
    patient_block = patient_context_block(summary)
...
def assemble_prompt(summary: dict, chunks: list[str], query: str) -> str:
    """Patient-specific RAG prompt with retrieved guideline chunks."""
    patient_block = patient_context_block(summary)
    evidence = "\n\n".join(
...
def assemble_general_prompt(chunks: list[str], query: str) -> str:
    """Guideline-only RAG prompt for non-patient-specific questions."""
    evidence = "\n\n".join(
        f"### Evidence block {i + 1} ###\n{chunk}"
        for i, chunk in enumerate(chunks)
    )
